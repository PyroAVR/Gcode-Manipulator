...
RS274::RS274(std::string inputFile, std::string outputFile) {
  ...
  while(!input.eof()) {
    ...
    linestoparse.push_back(inputBuffer);
    memset(&inputBuffer, 0, sizeof(inputBuffer));
  }
  int blockSize = linestoparse.size()/hardwarethreads;
  int numjobs = hardwarethreads;  //this could be changed later for batch jobs?
  jobs = numjobs; //pls fix
  if(linestoparse.size() < blockSize) numjobs = 1;

  //std::move??
  //also what the hell is all this iterator crap
  std::vector<std::string>::iterator separator = linestoparse.begin();
  separator += blockSize;
  threadWorkerData t;
  t.id = 0;
  t.size = blockSize;
...
int RS274::parse()  {
  for(int i = 0; i < tws.size(); i++) {
    threads[i] = std::thread(&RS274::runWorker, this, tws[i]);
  }
...
int RS274::parseRange(int start, int end) {
